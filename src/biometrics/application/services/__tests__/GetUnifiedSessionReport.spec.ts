...
describe('Unit | GetUnifiedSessionReportService', () => {
    ...
    it('should return NO_DATA_FOUND if no sessions exist', async () => {
        mockRepo.getFullSessionContext.mockResolvedValue({ sessions: [], intervals: [] });
    ...
    it('should calculate report metrics correctly', async () => {
        ...
        if (Array.isArray(result)) {
            expect(result[0].session_id).toBe('101');
            expect(result[0].dizziness_percentage).toBeGreaterThan(0);
        }
    });
});